...
class DashboardManifestLoaderTests(unittest.TestCase):
    def test_discovers_all_analysis_artifacts_and_pipeline_lanes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual([stage["id"] for stage in payload["stage_summaries"]], ["ingestion", "graph", "analysis", "visualization"])
            self.assertEqual(payload["stage_summaries"][0]["status"], "completed")
            self.assertEqual(payload["stage_summaries"][1]["label"], "Cohort Graph")
            self.assertEqual(payload["stage_summaries"][1]["status"], "completed")
            ...
            self.assertEqual(payload["stage_summaries"][3]["current"]["metrics"][0]["value"], 1)
            self.assertEqual(len(payload["stage_summaries"][2]["current"]["metrics"]), 2)
            self.assertEqual(len(payload["stage_summaries"][3]["current"]["metrics"]), 2)
